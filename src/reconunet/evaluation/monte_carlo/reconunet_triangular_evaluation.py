"""
Simple Evaluation Script for MUSIC and MVDR algorithms using Tri4Net Triangular Array Implementation

This script evaluates MUSIC and MVDR DOA estimation algorithms on Triangular Array
scenarios with 1, 2, and 3 sources, with controllable angle separation.
Uses Tri4Net components with triangular array geometry.
"""
...
class EvaluationParams:
    ...
    N: int = 4  # Number of sensors (fixed for triangular array)
    ...
    element_spacing: float = 0.5  # Element spacing for triangular array (in wavelengths)
    ...
    def __post_init__(self):
        ...
        # Force N=4 for triangular array
        if self.N != 4:
            print(f"Warning: Triangular array requires exactly 4 elements, setting N=4 (was {self.N})")
            self.N = 4


class Tri4NetTriangularEvaluator:
    """Simple evaluator for MUSIC and MVDR using Tri4Net Triangular Array"""
    ...
    def __init__(self, params: EvaluationParams):
        ...
        # Create array configuration for Triangular Array (Tri4Net)
        self.array_config = ArrayConfig(
            array_type='triangular',
            num_elements=4,  # Triangular array is always 4 elements
            carrier_freq=params.carrier_freq,
            element_spacing=params.element_spacing,  # Used for triangular array scaling
    ...
    def plot_results(self, results: Dict, save_path: str = None):
        ...
        for separation in all_separations:
            ...
            fig.suptitle(f'MUSIC vs MVDR Performance (Tri4Net Triangular) - {separation}° Angle Separation', fontsize=14)
            ...
            ax.set_title(f'RMSE vs SNR for {separation}° Separation (Triangular Array)', fontsize=12)
...
def main():
    ...
    # Set evaluation parameters for triangular array
    eval_params = EvaluationParams(
        N=4,  # 4-element triangular array (fixed)
        ...
        element_spacing=0.5  # Half wavelength spacing for triangular geometry
    )
    
    # Create evaluator
    evaluator = Tri4NetTriangularEvaluator(eval_params)
    
    print("Starting Tri4Net Triangular Array Evaluation...")
    print(f"Parameters: {eval_params.N} sensors (triangular), {eval_params.T} snapshots, {eval_params.num_monte_carlo} MC runs")
    
    # Run evaluation
    results = evaluator.run_comprehensive_evaluation()
    
    # Plot results (will generate one plot per angle separation)
    evaluator.plot_results(results, 'tri4net_triangular_evaluation')
    
    # Save results
    evaluator.save_results(results, 'tri4net_triangular_evaluation_results.json')
