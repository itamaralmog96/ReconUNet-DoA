"""
Simple Evaluation Script for MUSIC and MVDR algorithms using SubspaceNet ULA Implementation
...
subspacenet_path = os.path.join(os.path.dirname(__file__), '..', '..', '..', 'SubspaceNet', 'src')
if subspacenet_path not in sys.path:
    sys.path.insert(0, subspacenet_path)

# Import SubspaceNet components
try:
    ...
    print("Make sure SubspaceNet folder is in the correct location")
    sys.exit(1)
...
class EvaluationParams:
    ...
    T: int = 200  # Number of snapshots
    snr_range: List[float] = None  # SNR values to test
    num_monte_carlo: int = 100  # Number of Monte Carlo runs
    angle_separations: List[float] = None  # Angle separations to test (degrees)
    scan_range: Tuple[float, float] = (-90, 90)  # Scan range in degrees
    ...
    def __post_init__(self):
        ...
        if self.angle_separations is None:
            self.angle_separations = [5, 10, 15, 20, 30, 45]


class SubspaceNetULAEvaluator:
    """Simple evaluator for MUSIC and MVDR using SubspaceNet ULA"""
    ...
    def generate_test_angles(self, num_sources: int, angle_separation: float) -> np.ndarray:
        """Generate test angles with specified separation"""
        if num_sources == 1:
            return [0.0]  # Single source at broadside
        elif num_sources == 2:
            return [0.0, angle_separation]
        elif num_sources == 3:
            return [-angle_separation, 0.0, angle_separation]
        else:
            raise ValueError(f"Unsupported number of sources: {num_sources}")
    
    def create_test_signal(self, true_angles: np.ndarray, snr: float) -> Tuple[np.ndarray, SystemModel]:
        """Create test signal with given angles and SNR"""
        # Set system parameters
        system_params = SystemModelParams()
        system_params.M = len(true_angles)
        system_params.N = self.params.N
        system_params.T = self.params.T
        system_params.snr = snr
        system_params.signal_type = "NarrowBand"
        
        # Create samples
        samples_gen = Samples(system_params)
        samples_gen.set_doa(true_angles)
        
        X, _, _, _ = samples_gen.samples_creation()
        
        return X, samples_gen, len(true_angles)
    
    def estimate_doa_music(self, X: np.ndarray, system_model: SystemModel, num_sources: int) -> Tuple[np.ndarray, np.ndarray]:
        """Estimate DOA using MUSIC algorithm"""
        music = MUSIC(system_model)
        
        # Estimate DOAs
        estimated_angles, spectrum, M = music.narrowband(X, known_num_of_sources=True)
        
        return np.array(estimated_angles), spectrum
    
    def estimate_doa_mvdr(self, X: np.ndarray, system_model: SystemModel, M: int) -> Tuple[np.ndarray, np.ndarray]:
        """Estimate DOA using MVDR algorithm"""
        mvdr = MVDR(system_model)
        
        # Get MVDR response curve
        predictions, response_curve = mvdr.narrowband(X, mode="sample", eps=0.01)
        
        # Convert response curve to power (take real part and absolute value)
        spectrum = (np.real(response_curve))
        ...
        if len(peaks) == 0:
            # If no peaks found, take the maximum
            estimated_angles = np.array([mvdr._angels[np.argmax(spectrum)] * R2D])
        else:
            ...
            # estimated_angles = mvdr._angels[top_peaks] * R2D
            # Convert peak indices to angles
            angle_grid = np.rad2deg(mvdr._angels)
            mvdr_estimated_angles = angle_grid[top_peaks]
        
        return mvdr_estimated_angles, spectrum
    
    def estimate_doa_root_music(self, X: np.ndarray, system_model: SystemModel, num_sources: int) -> Tuple[np.ndarray, np.ndarray]:
        """Estimate DOA using Root MUSIC algorithm"""
        root_music = RootMUSIC(system_model)
        
        # Estimate DOAs using Root MUSIC
        estimated_angles, roots, estimated_angles_all, roots_angels_all, M = root_music.narrowband(X, known_num_of_sources=True)
        
        # Convert to numpy array if not already
        estimated_angles = np.array(estimated_angles)
        
        # Create a pseudo-spectrum from all angle predictions for visualization
        # This is not a true spectrum but helps with consistency in plotting
        spectrum = np.zeros_like(roots_angels_all)
        for i, angle in enumerate(estimated_angles_all):
            if i < len(spectrum):
                spectrum[i] = 1.0 / (1.0 + abs(angle))  # Simple inverse distance weighting
        
        return estimated_angles, spectrum
    
    def calculate_rmse(self, true_angles: np.ndarray, estimated_angles: np.ndarray) -> float:
    ...
    def run_single_scenario(self, num_sources: int, angle_separation: float, snr: float, 
        ...
        for _ in range(self.params.num_monte_carlo):
            try:
                ...
                X, system_model, M = self.create_test_signal(true_angles, snr)
                
                # Estimate DOAs
                if algorithm.upper() == 'MUSIC':
                    estimated_angles, _ = self.estimate_doa_music(X, system_model, num_sources)
                elif algorithm.upper() == 'MVDR':
                    estimated_angles, _ = self.estimate_doa_mvdr(X, system_model, M)
                elif algorithm.upper() == 'ROOT_MUSIC' or algorithm.upper() == 'ROOT-MUSIC':
                    estimated_angles, _ = self.estimate_doa_root_music(X, system_model, num_sources)
                else:
    ...
    def plot_results(self, results: Dict, save_path: str = None):
        ...
        for separation in all_separations:
            ...
            fig.suptitle(f'MUSIC vs MVDR vs Root-MUSIC Performance - {separation}° Angle Separation', fontsize=14)
...
def main():
    ...
    eval_params = EvaluationParams(
        N=8,  # 8-element ULA
        T=1024,  # 200 snapshots
        snr_range=np.arange(-10, 16, 1),
        num_monte_carlo=100,  # Reduced for faster execution
        angle_separations=[3, 5, 10],
        scan_resolution=1.0
    )
    
    # Create evaluator
    evaluator = SubspaceNetULAEvaluator(eval_params)
    
    print("Starting SubspaceNet ULA Evaluation...")
    print(f"Parameters: {eval_params.N} sensors, {eval_params.T} snapshots, {eval_params.num_monte_carlo} MC runs")
    # result = evaluator.run_single_scenario(2, 10, 10, 'MVDR')
    # Run evaluation
    results = evaluator.run_comprehensive_evaluation()
    # results_path = 'subspacenet_ula_evaluation_results.json'
    ...
    evaluator.plot_results(results, 'subspacenet_ula_evaluation')
    
    # Save results
    evaluator.save_results(results, 'subspacenet_ula_evaluation_results.json')
