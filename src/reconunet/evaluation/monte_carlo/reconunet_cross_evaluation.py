"""
Simple Evaluation Script for MUSIC and MVDR algorithms using Tri4Net Cross Array Implementation

This script evaluates MUSIC and MVDR DOA estimation algorithms on Cross Array
scenarios with 1, 2, and 3 sources, with controllable angle separation.
Uses Tri4Net components with cross array geometry.
"""
...
class EvaluationParams:
    ...
    N: int = 8  # Total number of elements in cross array
    ...
    scan_range: Tuple[float, float] = (0, 360)  # Scan range in degrees
    scan_resolution: float = 1.0  # Scan resolution in degrees
    carrier_freq: float = 2.45e9  # Carrier frequency in Hz
    fs: float = 1e3  # Sampling frequency in Hz
    element_spacing: float = 0.5  # Element spacing for cross array (in wavelengths)
    
    def __post_init__(self):
        if self.snr_range is None:
            self.snr_range = np.arange(-10, 16, 1)
        if self.angle_separations is None:
            self.angle_separations = [3, 5, 10]


class Tri4NetCrossEvaluator:
    """Simple evaluator for MUSIC and MVDR using Tri4Net Cross Array"""
    ...
    def __init__(self, params: EvaluationParams):
        ...
        # Create array configuration for Cross Array
        self.array_config = ArrayConfig(
            array_type='cross',
            num_elements=params.N,  # This determines the cross array size
        ...
        # With the new cross array logic, N directly specifies total elements
        self.actual_num_elements = params.N
        
        # Validate that N is valid for cross array
        if params.N % 2 == 1:  # Odd - must be 4k+1
            if (params.N - 1) % 4 != 0:
                suggested1 = 4*((params.N-1)//4)+1
                suggested2 = 4*((params.N-1)//4)+5
                raise ValueError(f"For odd N, must be of form 4k+1. Got N={params.N}. Try N={suggested1} or N={suggested2}")
        else:  # Even - must be 4k
            if params.N % 4 != 0:
                suggested1 = 4*(params.N//4)
                suggested2 = 4*(params.N//4+1)
                raise ValueError(f"For even N, must be divisible by 4. Got N={params.N}. Try N={suggested1} or N={suggested2}")
        
        print(f"Cross Array Configuration: N={params.N} total elements")
    ...
    def create_test_signal(self, true_angles: List[float], snr: float) -> Tuple[np.ndarray, ArrayModel]:
        ...
        signal_config = SignalConfig(
            ...
            angles=true_angles,
            frequencies=[1e2] * len(true_angles),  # Adjusted frequency for lower fs
            snr_db=snr,
    ...
    def run_single_scenario(self, num_sources: int, angle_separation: float, snr: float, 
        ...
        return {
            ...
            'array_elements': self.actual_num_elements
        }
    ...
    def run_comprehensive_evaluation(self) -> Dict:
        ...
        total_scenarios = len([1, 2, 3]) * len(self.params.angle_separations) * len(self.params.snr_range) * 2
        scenario_count = 0
        
        for algorithm in ['MUSIC', 'MVDR']:
    ...
    def plot_results(self, results: Dict, save_path: str = None):
        ...
        for separation in all_separations:
            ...
            fig.suptitle(f'MUSIC vs MVDR Performance (Tri4Net Cross Array) - {separation}° Angle Separation', fontsize=14)
            ...
            ax.set_title(f'RMSE vs SNR for {separation}° Separation (Cross Array, N={self.params.N}, {self.actual_num_elements} elements)', fontsize=12)
    ...
    def save_results(self, results: Dict, save_path: str):
        ...
        json_results['metadata'] = {
            'array_type': 'cross',
            'N_parameter': self.params.N,
            'actual_num_elements': self.actual_num_elements,
            'element_spacing': self.params.element_spacing,
            'carrier_freq': self.params.carrier_freq,
            'fs': self.params.fs,
            'T': self.params.T,
            'num_monte_carlo': self.params.num_monte_carlo
        }
...
def main():
    ...
    # Set evaluation parameters for cross array
    eval_params = EvaluationParams(
        N=17,  # Total number of elements (9 = 1 center + 2 per arm)
        T=1024,  # 1024 snapshots
        snr_range=np.arange(-10, 20, 1),
        num_monte_carlo=100,  # Number of Monte Carlo runs
        angle_separations=[5, 10, 20],
        scan_resolution=1.0,
        carrier_freq=2.45e9,  # 2.45 GHz
        fs=1e3,  # 1 kHz sampling frequency
        element_spacing=1/3  # Half wavelength spacing
    )
    
    # Create evaluator
    evaluator = Tri4NetCrossEvaluator(eval_params)
    
    print("Starting Tri4Net Cross Array Evaluation...")
    print(f"Parameters: {eval_params.N} total elements, {eval_params.T} snapshots, {eval_params.num_monte_carlo} MC runs")
    
    # Run evaluation
    results = evaluator.run_comprehensive_evaluation()
    
    # Plot results (will generate one plot per angle separation)
    evaluator.plot_results(results, 'tri4net_cross_evaluation')
    
    # Save results
    evaluator.save_results(results, 'tri4net_cross_evaluation_results.json')
